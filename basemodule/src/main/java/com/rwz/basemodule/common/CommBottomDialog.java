...
import com.rwz.basemodule.databinding.DialogBottomLinearBinding;
...
public class CommBottomDialog extends BaseDialog<DialogBottomLinearBinding> implements View.OnClickListener {
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        StringHelp.judgeNull(mTitle, title, true);
        if (mData == null) {
    ...
    public static class Build {
        ...
        public Build addItem(String content) {
            if(!TextUtils.isEmpty(content))
                addItem(new CommBottomEntity(content, true));
            return this;
        }
        ...
        public CommBottomDialog create(List<String> data) {
            ...
            if (data != null && data.size() > 0) {
                for (int i = 0; i < data.size(); i++) {
                    CommBottomEntity entity = new CommBottomEntity(data.get(i), true);
                    list.add(entity);
                }
            }
            return create(list);
        ...
        public CommBottomDialog create(String[] data) {
            ...
            if (data != null && data.length > 0) {
                for (String aData : data) {
                    CommBottomEntity entity = new CommBottomEntity(aData, true);
                    list.add(entity);
                }
            }
            return create(list);
