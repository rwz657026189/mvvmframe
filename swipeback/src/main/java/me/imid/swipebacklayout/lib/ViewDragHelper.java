    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                // report edge drags.
                final int pointerCount = MotionEventCompat.getPointerCount(ev);
                //避免快速滑动的时候NullPointerException(如：声币、声豆列表)
                if (mInitialMotionX == null || mInitialMotionY == null) {
                    Log.e(TAG, "shouldInterceptTouchEvent: " + "mInitialMotionX = " + Arrays.toString(mInitialMotionX) + ", mInitialMotionY = " + Arrays.toString(mInitialMotionY));
                    break;
                }
                for (int i = 0; i < pointerCount; i++) {
                    final int pointerId = MotionEventCompat.getPointerId(ev, i);
